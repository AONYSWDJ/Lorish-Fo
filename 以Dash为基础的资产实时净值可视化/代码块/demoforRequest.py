...
class CallBack(XtTraderApiCallback):
    ...
    # 查询账号历史资金信息
    def reqHistoryAccountDetailSync(self, account_id, account_key,start_Date,end_Date):
        print("reqHistoryAccountDetailSync started")
        errno = XtError(0, "")
        deallist = self.m_api.reqHistoryAccountDetailSync(account_id, start_Date, end_Date, errno, account_key)
        if errno.isSuccess:
    ...
    def rePositionStaticsSyncWithProductId(self, ProductId,):
        print("依据产品得到产品旗下的所有账号，第一个新函数:",ProductId)

        errno = XtError(0, "")
        AccountListContainsAll = self.m_api.reqPositionStaticsSyncWithProductId(errno, ProductId)
        if errno.isSuccess:
            print("reqDealStaticsSync success, record:", len(AccountListContainsAll))
            # for data in dealStaticsList:
            #     print("m_strInstrumentID:", data.m_strInstrumentID, "m_nVolume:", data.m_nVolume)
        return AccountListContainsAll
    #同步查询指令对应成交数据
    ...
    #同步查询历史持仓统计
    def reqHistoryPositionStaticsSync(self, account_id, account_key,start_Date,end_Date):
        print("reqHistoryDealDetailSync started")
        errno = XtError(0, "")
        deallist = self.m_api.reqHistoryPositionStaticsSync(account_id, start_Date, end_Date, errno, account_key)
        if errno.isSuccess:
